...
public class MethodsOfSession {
	...
	1，hibernate三種狀態
		...
		Detached(遊離狀態)：持久化物件脫離了Session後的物件。如Session快取被清空後的物件。已經持久化，單不存在於Session中——此狀態中的物件為遊離物件。
	2，三種狀態的區分：
		...
		建立一個Teacher物件，此時記憶體中有了這個物件，但是沒有Id，快取中也沒有，為Transient狀態；
		將Teacher物件通過save()方法儲存到資料庫之後
		在Session快取裡就有了這個物件(快取了這個獨享的Id，由這個Id指向它)
		並且資料庫也有了此物件(有Id)，為Persistent狀態；Session物件關閉，Teacher物件在記憶體中依舊存在
		Session快取中不再存在，為Detached物件。
		Transient(臨時)物件與Detached(遊離)物件的異同之處：
	...
	*/
	...
	public void saveDemo01() {
		...
		BookBean bb1 = new BookBean(1000, "美麗人生-1", "孫玲", 450.0, 150);
		BookBean bb2 = new BookBean(null, "健康人生-1", "張安國", 350.0, 100);
		BookBean bb3 = new BookBean(null, "幸福人生-1", "楊磊", 380.0, 325);
	...
	public void saveDemo02() {
		BookBean bb1 = new BookBean(1000, "美麗人生-續集-2", "孫玲", 450.0, 100);
		BookBean bb2 = new BookBean(null, "健康人生-續集-2", "張安國", 350.0, 80);
		BookBean bb3 = new BookBean(null, "幸福人生-續集-2", "楊磊", 380.0, 75);
		// save()的傳回值型態為java.io.Serializable  傳回值是物件的主鍵值
		Serializable o = session.save(bb1); // 儲存後bb1為永續物件
		System.out.println("Key=" + o);  // Key=1
		o = session.save(bb2); // 儲存後bb2為永續物件
		System.out.println("Key=" + o);     
		o = session.save(bb3); // 儲存後bb3為永續物件
		System.out.println("Key=" + o);
		// 這行敘述會丟出例外
		//bb1.setBookId(1000);   // 除非刪掉 @GeneratedValue(strategy = GenerationType.IDENTITY)
	...
	public void persistDemo01() {
		...
		session.persist(bb1);
		session.persist(bb2);
		bb1.setAuthor("胖虎");   // 永續物件位於Session快取中，受Hibernate監控，修改永續物件不需要
	}							// 執行Session介面的update()方法
...
//	1. get()會立即由表格中讀取對應的紀錄(立即載入)，
//	   load()不會立即由表格中讀取對應的紀錄(延遲載入，Lazy Loading)而會傳回一個代理類別的物件
...
// get()方法會去讀取資料庫，傳回要讀取的永續物件。如果永續物件不存在，程式會傳回null。
	...
	public void getDemo01() {
	    BookBean bb1 = session.get(BookBean.class, 8);   // 將物件的主鍵值改為8再試一次
	    if (bb1 != null) {
	        System.out.println(bb1);  // BookBean如有設置toString 則默認調用此方法bb1.toString
	    } else {
...
//	--------------------------------------------------------	
	...
	public void loadDemo01() {
		BookBean bb1 = session.load(BookBean.class, 9);
		System.out.println("load()執行完畢...");
		System.out.println("----------------------------------------");
		System.out.println("代理類別的Id: " + bb1.getBookId());
		System.out.println("----------------------------------------");
	...
	public void flushDemo01() {
		BookBean bb1 = session.get(BookBean.class, 4);
		System.out.println("select --> 完畢");
		if (bb1 == null) {
		...
		session.flush();        // commit()交易前執行       
//		session.update(bb1);    // commit()交易開始執行   
...
//	--------------------------------------------------------
	...
	public void deleteDemo04() {
		...
		session.delete(bb2);  
	  //session.update(bb2);  則除了主鍵 其他都是null
	}
	...
	@Test
	// Session#update() 用來更新臨時物件會在執行tx.commit()時丟出例外。
	public void updateDemo02() {
		BookBean bb = new BookBean(null, "古代詩詞欣賞", "張書豪", 520.0, 37);
...
// 如果傳入的參數為 臨時(transient)物件，執行新增操作，執行INSERT敘述
	...
	// session.merge(bb1);與session.save(bb1)效果一樣 此物件為transient物件
	...
	public void mergeDemo02() {
		...
		BookBean bb2 = new BookBean(12, "當Sally遇見Hally第1集", "李建國", 500.0, 14);
...
//		session.flush();
	...
	public void mergeDemo03() {
		...
		BookBean bb3 = new BookBean(300, "當Sally遇見Hally-3", "李建國-303", 410.0, 303);
	...
	// ------------------------------------------------------------------------
	/*
	 * session.merge(obj): 本方法會傳回更新或新增後的物件，此物件為永續物件。 傳入的參數不會是永續物件，而可能是： 
	 * 1.分離物件(具有效的主鍵值，更新它) 2. 臨時物件(OID為 null，或沒有對應的主鍵值，新增它)
	 */

	@Test
	public void mergeDemo04() {
		// bb1有OID, 而且表格內有紀錄與之對應(即存在一筆記錄，其主鍵值與OID相等)，此時，
		// 此物件為detached物件, 接著執行session.merge(bb2);時，會發UPDATE的SQL敘述。
		BookBean bb1 = new BookBean(1, "健康人生第二集", "張美村", 380.0, 25);
		BookBean bbR1 = (BookBean) session.merge(bb1);
		System.out.println("---------------bb1------------");
		
		// bb2沒有OID, 執行session.merge(bb1)，會發出INSERT INTO的SQL敘述，此時session.merge(bb2)
		// 與session.save(bb2)效果一樣  此物件為transient物件
		BookBean bb2 = new BookBean(null, "健康人生第三集", "張大村", 390.0, 27);
		BookBean bbR2 = (BookBean) session.merge(bb2);
		System.out.println("---------------bb2------------");
		
		// bb3有OID, 但表格內沒有紀錄與之對應(即沒有任何一筆記錄，其主鍵值與OID相等)，此物件為transient物件
		// 接著執行session.merge(bb3);時，會發INSERT INTO的SQL敘述
		BookBean bb3 = new BookBean(1000, "健康人生第四集", "張小村", 400.0, 20);
...
//		 bb3.setAuthor("張春芳-333(參數)");
	...
	@Test
	public void clear01() {
		BookBean bb1 = session.get(BookBean.class, 1);
		System.out.println("bb1的作者: " + bb1.getAuthor());
		//session.clear();
		System.out.println("----------------------------");
		bb1 = session.get(BookBean.class, 1);
		System.out.println("bb1的作者: " + bb1.getAuthor());
	...
	public void clear02() {
		BookBean bb1 = session.get(BookBean.class, 1);
		BookBean bb2 = session.get(BookBean.class, 2);
		System.out.println("bb1的作者: " + bb1.getAuthor());
	...
	public void evict() {
		BookBean bb1 = session.get(BookBean.class, 1);
		BookBean bb2 = session.get(BookBean.class, 2);
		BookBean bb3 = session.get(BookBean.class, 3);
		bb1.setAuthor("孫悟空");
		bb2.setAuthor("朱八戒");
		bb3.setAuthor("沙悟淨");
		session.evict(bb1);
